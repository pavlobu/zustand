...
export const useWolvesStore = create<WolvesState>()(devtools(immer((set) => ({
  wolves: 0,
  increasePopulation: () => set((store) => {
    store.wolves += 1;
  }, false, { type: 'increasePopulation' }),
  removeWolf: () => set((store) => {
    store.wolves -= 1;
  }, false, { type: 'removeWolf' }),
  removeAllWolves: () => set((store) => {
    store.wolves = 0;
  }, false, { type: 'removeAllWolves' }),
  setSpecificWolvesAmount: (amount: number) => set((store) => {
    store.wolves = amount;
  }, false, { type: 'setSpecificWolvesAmount' }),
